...
public class BJCommonImageCropHelper {
     ...
     * 进入相册，选择多图
     * @param context
     * @param maxSize  最大选择图片数;输入100则不显示最大数
     * @param themeConfig 样式配置
     * @param callback
     */
    public static void openImageMulti(Context context, List<PhotoInfo> photoList,int maxSize, ThemeConfig themeConfig, OnHandlerResultCallback callback) {
        if (maxSize <= 0) return;
        ...
        PhotoSelectActivity.launch(context,photoList);
    }
